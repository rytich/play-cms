...
- Node.js `^22.13.0 || >=24.0.0`、pnpm `11.19.0`以上という既存要件を維持。lockfile・依存・API契約・DB schemaを変更しない。
...
**Files:** 上記のCreate / Modify / Testを対象とする。サーバー・DB・認証実装は対象外。
...
AdminVideosの表示状態を`list`/`editor`に限定し、一覧の右上に「動画を登録」、行に「編集」、編集画面に「一覧へ戻る」を配置する。既存ページ送りと選択対象を保持し、画面変更後に対応見出しへフォーカスする。AdminVideos内の`returnToList(): void`を、未保存確認後に一覧へ切り替えるhandlerとし、AdminLayoutの`onVideos`と「一覧へ戻る」ボタンから同じhandlerを呼ぶ。DOM検索で隠れたボタンを押すような間接接続はしない。
