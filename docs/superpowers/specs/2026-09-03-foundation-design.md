...
- Task 8でCIを導入するまでは、ローカル検証のコマンドと結果をIssueとPRへ記録し、独立レビューをマージ条件とする。Task 8のruleset有効化前は自動Mergeを禁止する。導入後はrulesetでstale Approve無効化、branch最新化、CI必須化、`knryt`のbypass禁止を強制し、required指定の有無にかかわらずreview対象headのCIチェックが存在し、すべて成功していることを必須とする。チェック0件は成功扱いにしない。
