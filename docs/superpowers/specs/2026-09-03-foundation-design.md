...
- 公開コードでも成立する`deny-by-default`の防御を採用し、実装の秘匿性に依存しない。
...
標準機能で実現できる防御のために独自暗号や新しい依存を追加しない。入力は境界で検証し、認証、認可、期限、使い切り状態をサーバー側で確認する。ログイン、視聴コード入力、外部APIなど自動化された試行や負荷の対象になる処理には、CloudflareとNode.jsで同じ拒否条件となるレート制限を設ける。

秘密値を伴う外部通信は送信先を固定し、リダイレクトを拒否する。使い切り視聴コードはハッシュだけを保存して原子的に消費する。視聴期限の終了後は動画本体、タイトル、説明、サムネイル、字幕、署名URLを返さず、キャッシュやストレージURLからの迂回も許可しない。具体的な実装と負系テストは、それぞれの機能Taskで設計する。
