...
- Filmaでアップロード済みの動画をFilmaファイルID、タイトル、説明、公開／非公開（下書き）、任意の公開開始・終了日時で登録・編集できる。開始だけ、終了だけ、両方なしも選べる。
- 閲覧キーを必要な数だけ一件ずつ発行できる。キーは一人一コードの使い切りとし、管理者は選択した一件だけ再表示できる。旧キーを再利用不能にして履歴を残す再発行は、未使用・無効・取消済み・使用済みの各状態で行える。
- 既存動画の編集画面から同一サイトの公開URLを開き、コピーできる。
