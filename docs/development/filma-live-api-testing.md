...
このコマンドだけが`POST https://filma.biz/filmaapi/token`を実行します。リダイレクトは拒否します。200応答では`organization_id`と`api_type`の存在と型を検証し、出力にはHTTPステータスと確認した項目名だけを表示します。APIキー、JWT、組織ID、認証ヘッダー、レスポンス本文は表示しません。
