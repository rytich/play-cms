...
2026-09-07、ユーザーは実動画公開を無効にしたまま管理画面・動画登録・閲覧用キー管理を先行する案を承認した。「使い切りコード」はソースコードではなく閲覧用キーであり、発行総数を少数に制限しない。当初は一件ずつの追加発行に限定し、一括生成・CSV出力・再表示を含めず着手した。その後の現行境界は以下に記す。
...
- 視聴者向けの動画情報と再生URLは、公開状態・任意の公開期間・使い切りキーによる引換または既存視聴権をサーバーで確認した場合だけ返す。下書き、期間外、未認可では存在やタイトルを開示しない。
...
Issue #32/#35/#45で、視聴者認証・視聴権、使い切りキー消費、APIキーの暗号化保存、認可済み再生URLの受け渡しまでを追加した。これらは合成契約でfail closedを確認する段階であり、実Filma契約の確認、アップロード、サムネイル、通知、一般公開は引き続き範囲外とする。DBは使用するテーブルだけを追加し、単純なSQLとprepared statementを使う。
...
- [x] Worker/D1のmigration、管理者・視聴者認証、下書き／公開状態、視聴権、キー管理API
- [x] 日本語の管理画面と、一覧には生キーを含めない単一キーの暗号化再表示・再発行
- [x] 公開状態・任意期間・キー消費または視聴権による認可と、未認可時の非開示を合成テストで確認
- [ ] 実Filma契約、Workers Free実測、Issue #30のブラウザゲートを確認してから試験公開を判断

Issue #18の先行縦切りからIssue #32/#35/#45へ段階的に拡張した。実Filma契約、Workers Free実測、試験公開の条件が満たされるまで、一般公開可能なP0の完成とは報告しない。
